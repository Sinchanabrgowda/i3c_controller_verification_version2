`ifndef I3C_SDR_WRITE_VIRTUAL_SEQ_LEN_1_WDATA_55_INCLUDED_
`define I3C_SDR_WRITE_VIRTUAL_SEQ_LEN_1_WDATA_55_INCLUDED_
class i3c_sdr_write_virtual_seq_len_1_wdata_55 extends top_virtual_base_seq;
  `uvm_object_utils(i3c_sdr_write_virtual_seq_len_1_wdata_55)
  uvm_status_e   status;
  ...
  // -----------------------------------------------------------------
  // FIXED -- original paste was missing ';' after 8'h55 before the
  // foreach block's closing brace:
  //   wdata_bytes[i] == 8'h55
  //   };
  ...
  function new(string name = "i3c_sdr_write_virtual_seq_len_1_wdata_55");
    super.new(name);
  endfunction
  task body();
    ...
    for (int i = 0; i < transfer_len; i++) begin
      `uvm_info(get_type_name(),
        $sformatf("Pushing WDATAB[%0d] = 0x%02h", i, wdata_bytes[i]),
        UVM_LOW)
    ...
    `uvm_info(get_type_name(),
      ...
      UVM_LOW)
  endtask : body
endclass : i3c_sdr_write_virtual_seq_len_1_wdata_55
`endif

`ifndef I3C_SDR_WRITE_READ_TRANSFER_LEN_16_VIRTUAL_SEQ_INCLUDED_
`define I3C_SDR_WRITE_READ_TRANSFER_LEN_16_VIRTUAL_SEQ_INCLUDED_
class i3c_sdr_write_read_transfer_len_16_virtual_seq extends top_virtual_base_seq;
  `uvm_object_utils(i3c_sdr_write_read_transfer_len_16_virtual_seq)
  uvm_status_e   status;
  ...
  function new(string name = "i3c_sdr_write_read_transfer_len_16_virtual_seq");
    super.new(name);
  endfunction
  task body();
    ...
    // monitor proxy the next START is an SDR frame, not another DAA
    // broadcast (has_daa stays 1 for the whole test). Must be set again
    // before the READ block below too -- the proxy clears pending_sdr
    // back to 0 as soon as it finishes sampling *this* frame (the
    // write), so it needs to be re-asserted before the second,
    // independent SDR frame (the read) starts. Same pattern as
    // i3c_sdr_write_read_multi_transfer_len_virtual_seq.sv.
    // -----------------------------------------------------------------
    ...
    // -----------------------------------------------------------------
    // pending_sdr -- ADDED. Re-assert before the READ frame -- this is a
    // separate START...STOP transaction from the write above.
    // -----------------------------------------------------------------
    i3c_env_cfg_h.i3c_target_agent_cfg_h[target_idx].pending_sdr = 1;
    fork
      begin
    ...
    `uvm_info(get_type_name(),
      ...
      UVM_LOW)
  endtask : body
endclass : i3c_sdr_write_read_transfer_len_16_virtual_seq
`endif

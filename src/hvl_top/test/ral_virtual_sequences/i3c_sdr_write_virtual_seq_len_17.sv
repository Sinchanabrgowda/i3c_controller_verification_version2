`ifndef I3C_SDR_WRITE_VIRTUAL_SEQ_LEN_17_INCLUDED_
`define I3C_SDR_WRITE_VIRTUAL_SEQ_LEN_17_INCLUDED_
class i3c_sdr_write_virtual_seq_len_17 extends top_virtual_base_seq;
  `uvm_object_utils(i3c_sdr_write_virtual_seq_len_17)
  uvm_status_e   status;
  uvm_reg_data_t ctrl_val;
  uvm_reg_data_t ctrl_mirror;
  int unsigned target_idx = 0;
  // -----------------------------------------------------------------
  // FIXED -- original paste had "data_bytes [0:16];" with no type and
  // no name prefix (truncated mid-edit). Restored as a properly
  // declared 17-element randomized byte array matching transfer_len==17.
  // -----------------------------------------------------------------
  ...
  function new(string name = "i3c_sdr_write_virtual_seq_len_17");
    super.new(name);
  endfunction
  task body();
    ...
    `uvm_info(get_type_name(),
      ...
      UVM_LOW)
    // ------------------------------------------------------------------
    // Step 3: program CTRL to kick off the SDR transaction
    // ------------------------------------------------------------------
    ...
    `uvm_info(get_type_name(),
      ...
      UVM_LOW)
  endtask : body
endclass : i3c_sdr_write_virtual_seq_len_17
`endif

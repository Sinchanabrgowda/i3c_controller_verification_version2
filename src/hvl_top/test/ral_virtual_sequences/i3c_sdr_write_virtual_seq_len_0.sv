`ifndef I3C_SDR_WRITE_VIRTUAL_SEQ_LEN_0_INCLUDED_
`define I3C_SDR_WRITE_VIRTUAL_SEQ_LEN_0_INCLUDED_
class i3c_sdr_write_virtual_seq_len_0 extends top_virtual_base_seq;
  `uvm_object_utils(i3c_sdr_write_virtual_seq_len_0)
  uvm_status_e   status;
  ...
  function new(string name = "i3c_sdr_write_virtual_seq_len_0");
    super.new(name);
  endfunction
  task body();
    ...
    // i3c_sdr_write_virtual_seq / _nack_ / _multi_transfer_len_. Tells
    // the monitor proxy the next START is an SDR frame, not another DAA
    // broadcast (has_daa stays 1 for the whole test).
    // -----------------------------------------------------------------
    ...
    `uvm_info("CTRL_DEBUG",
      $sformatf("CTRL mirrored after write = 0x%0h", ctrl_mirror),
      UVM_LOW)
    ...
    `uvm_info(get_type_name(),
      ...
      UVM_LOW)
  endtask : body
endclass : i3c_sdr_write_virtual_seq_len_0
`endif

`ifndef I3C_SDR_WRITE_READ_MULTI_TRANSFER_LEN_VIRTUAL_SEQ_INCLUDED_
`define I3C_SDR_WRITE_READ_MULTI_TRANSFER_LEN_VIRTUAL_SEQ_INCLUDED_
class i3c_sdr_write_read_multi_transfer_len_virtual_seq extends top_virtual_base_seq;
  `uvm_object_utils(i3c_sdr_write_read_multi_transfer_len_virtual_seq)
  uvm_status_e   status;
  ...
  function new(string name = "i3c_sdr_write_read_multi_transfer_len_virtual_seq ");
    super.new(name);
  endfunction
  task body();
    ...
    for (int i = 0; i < transfer_len; i++)
      `uvm_info(get_type_name(),
        ...
        UVM_LOW)
    // pending_sdr -- ADDED. See notes below for why it's re-asserted twice.
    ...
    // pending_sdr -- ADDED. Re-assert before the READ frame -- the proxy
    // already cleared it back to 0 once the write frame finished
    // sampling, and the read below is a separate START...STOP
    // transaction that needs the gate open again.
    i3c_env_cfg_h.i3c_target_agent_cfg_h[target_idx].pending_sdr = 1;
    fork
      begin
        target_seq_read =
    ...
    i3c_env_cfg_h.regBlockHandle.ctrl_inst.cmd_type.set(2'd1);  //private read SDR
    i3c_env_cfg_h.regBlockHandle.ctrl_inst.cmd_ccc.set(8'd0);
    i3c_env_cfg_h.regBlockHandle.ctrl_inst.cmd_mode.set(1'b0);
    i3c_env_cfg_h.regBlockHandle.ctrl_inst.start.set(1'b1);
    ctrl_val = i3c_env_cfg_h.regBlockHandle.ctrl_inst.get();
    ...
    `uvm_info(get_type_name(),
      ...
      UVM_LOW)
  endtask : body
endclass : i3c_sdr_write_read_multi_transfer_len_virtual_seq
`endif

`ifndef I3C_SDR_WRITE_READ_TRANSFER_LEN_16_WDATA_FFFF_VIRTUAL_SEQ_INCLUDED_
`define I3C_SDR_WRITE_READ_TRANSFER_LEN_16_WDATA_FFFF_VIRTUAL_SEQ_INCLUDED_
class i3c_sdr_write_read_transfer_len_16_wdata_FFFF_virtual_seq extends top_virtual_base_seq;
  `uvm_object_utils(i3c_sdr_write_read_transfer_len_16_wdata_FFFF_virtual_seq)
  uvm_status_e   status;
  ...
  function new(string name = "i3c_sdr_write_read_transfer_len_16_wdata_FFFF_virtual_seq");
    super.new(name);
  endfunction
  task body();
    ...
    for (int i = 0; i < transfer_len; i++) begin
      `uvm_info(get_type_name(),
        $sformatf("WRITE: pushing WDATAB[%0d] = 0x%02h",
                  i, wdata_bytes[i]),
        UVM_LOW)
      i3c_env_cfg_h.regBlockHandle.wdatab_inst.write(
        status,
        uvm_reg_data_t'(wdata_bytes[i]),
        UVM_FRONTDOOR,
        .parent(this));
      if (status != UVM_IS_OK)
        `uvm_error(get_type_name(),
          $sformatf("WDATAB write[%0d]=0x%02h failed",
                    i, wdata_bytes[i]))
    end
    `uvm_info(get_type_name(),
      $sformatf("WRITE: all %0d bytes loaded into WDATAB FIFO",
                transfer_len),
      UVM_LOW)
    ...
    #(5000 * (transfer_len + 2));
    `uvm_info(get_type_name(),
      $sformatf("WRITE to target[%0d] complete", target_idx),
      UVM_LOW)
    // -----------------------------------------------------------------
    ...
    `uvm_info(get_type_name(),
      ...
      UVM_LOW)
  endtask : body
endclass : i3c_sdr_write_read_transfer_len_16_wdata_FFFF_virtual_seq
`endif

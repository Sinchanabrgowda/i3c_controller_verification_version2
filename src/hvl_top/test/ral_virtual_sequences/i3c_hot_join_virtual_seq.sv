...
class i3c_hot_join_virtual_seq extends top_virtual_base_seq;
  ...
  task body();
    ...
    // FIX: tracks real completion of each forked initial-DAA target thread
    ...
    // Targets must still be launched concurrently (fork/join_none) -- that
    // is the whole point of DAA arbitration, all competing devices need to
    // be driving/sampling the bus at the same time. What changes is that we
    // now actually wait for every one of them to finish below, instead of
    // sleeping a fixed guessed duration and moving on regardless.
    ...
    // FIX: was `#(wait_ns * 1ns);` with wait_ns = timeout_per_slave_ns *
    // hot_join_idx, then unconditionally proceeding regardless of whether
    // the 3 forked DAA sequences had actually finished. Now we poll the
    // done-flags and only proceed once all hot_join_idx targets report
    // done, or a generous timeout elapses (safety net, not the normal
    // path).
    begin
      bit all_daa_done;

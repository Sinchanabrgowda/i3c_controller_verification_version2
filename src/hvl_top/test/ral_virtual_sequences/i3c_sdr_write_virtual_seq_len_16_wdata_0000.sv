`ifndef I3C_SDR_WRITE_VIRTUAL_SEQ_LEN_16_WDATA_0000_INCLUDED_
`define I3C_SDR_WRITE_VIRTUAL_SEQ_LEN_16_WDATA_0000_INCLUDED_
class i3c_sdr_write_virtual_seq_len_16_wdata_0000 extends top_virtual_base_seq;
  `uvm_object_utils(i3c_sdr_write_virtual_seq_len_16_wdata_0000)
  uvm_status_e   status;
  ...
  // -----------------------------------------------------------------
  // FIXED -- original paste had a syntax error here:
  //   foreach (wdata_bytes[i]) {
  //   wdata_bytes[i] == 8'h00};
  //   }
  // missing ';' terminating the constraint expression before the
  // foreach block's closing brace, plus a stray ';' after that brace.
  ...
  function new(string name = "i3c_sdr_write_virtual_seq_len_16_wdata_0000");
    super.new(name);
  endfunction
  task body();
    ...
    `uvm_info(get_type_name(),
      ...
      UVM_LOW)
  endtask : body
endclass : i3c_sdr_write_virtual_seq_len_16_wdata_0000
`endif

`ifndef I3C_SDR_WRITE_READ_TRANSFER_LEN_16_WDATA_0_VIRTUAL_SEQ_INCLUDED_
`define I3C_SDR_WRITE_READ_TRANSFER_LEN_16_WDATA_0_VIRTUAL_SEQ_INCLUDED_
class i3c_sdr_write_read_transfer_len_16_wdata_0_virtual_seq extends top_virtual_base_seq;
  `uvm_object_utils(i3c_sdr_write_read_transfer_len_16_wdata_0_virtual_seq)
  uvm_status_e   status;
  ...
  function new(string name = "i3c_sdr_write_read_transfer_len_16_wdata_0_virtual_seq");
    super.new(name);
  endfunction
  task body();
    ...
    // as the other write+read sequences. Must be set again before the
    // READ block below too -- the proxy clears pending_sdr back to 0
    // once it finishes sampling the write frame.
    // -----------------------------------------------------------------
    ...
    `uvm_info(get_type_name(),
      ...
      UVM_LOW)
  endtask : body
endclass : i3c_sdr_write_read_transfer_len_16_wdata_0_virtual_seq
`endif

  ...
  forever begin
    for (int i = 0; i < num_targets; i++) begin
      if (ibi_analysis_fifo[i].try_get(tgt_i)) begin
        ...
        target_tx_count++;
        `uvm_info("SB_IBI",
          $sformatf("[target %0d] IBI result: addr=0x%0h ack=%0b mdb=0x%0h T1=%0b extra_bytes_sent=%0d",
                    i, tgt_i.dynamic_address, tgt_i.daa_ack, tgt_i.ibi_mdb,
                    tgt_i.ibi_t1, tgt_i.ibi_extra_data_sent.size()),
          UVM_LOW)
        if (tgt_i.daa_ack !== ACK) begin
        ...
        // -- T-bit / N extra data bytes (additive, non-breaking check).
        // Per the I3C spec the TARGET drives the T-bit for IBI payload,
        // so T1 here reflects what this target itself decided/drove.
        // Generalized -- NEW: loops over however many extra bytes were
        // actually sent instead of a single hardcoded 2nd byte.
        `uvm_info("SB_IBI_TBIT",
          $sformatf("[target %0d] IBI T-bit driven by target after MDB: T1=%0b (%0s)",
                    i, tgt_i.ibi_t1,
                    tgt_i.ibi_t1 ? "more data follows" :
                                   "end of data (STOP)"),
          UVM_MEDIUM)
        if (tgt_i.ibi_extra_data_sent.size() > 0) begin
          bit [7:0] exp_extra_data[];
          exp_extra_data = i3c_env_cfg_h.i3c_target_agent_cfg_h[i].ibi_extra_data;
          foreach (tgt_i.ibi_extra_data_sent[k]) begin
            bit [7:0] exp_val;
            bit       exp_t;
            exp_val = (k < exp_extra_data.size()) ? exp_extra_data[k] : 8'hFF;
            if (tgt_i.ibi_extra_data_sent[k] !== exp_val) begin
              `uvm_error("SB_IBI_EXTRA_DATA",
                $sformatf("[target %0d] IBI extra byte[%0d] mismatch: expected 0x%0h got 0x%0h",
                          i, k, exp_val, tgt_i.ibi_extra_data_sent[k]))
              ibi_fail++;
            end else begin
              `uvm_info("SB_IBI_EXTRA_DATA",
                $sformatf("[target %0d] IBI extra byte[%0d] 0x%0h PASS",
                          i, k, tgt_i.ibi_extra_data_sent[k]), UVM_MEDIUM)
              ibi_pass++;
            end
            // Every extra byte except the last must carry T=1; the last
            // one must carry T=0.
            exp_t = (k < tgt_i.ibi_extra_data_sent.size() - 1) ? 1'b1 : 1'b0;
            if (tgt_i.ibi_extra_t_bits[k] !== exp_t) begin
              `uvm_error("SB_IBI_EXTRA_TBIT",
                $sformatf("[target %0d] IBI extra byte[%0d] T-bit mismatch: expected %0b got %0b",
                          i, k, exp_t, tgt_i.ibi_extra_t_bits[k]))
              ibi_fail++;
            end
          end
        end else if (i3c_env_cfg_h.i3c_target_agent_cfg_h[i].ibi_num_extra_bytes > 0) begin
          // The target drives its T-bits unconditionally once the IBI
          // request is ACKed, so this branch should only be reachable if
          // the transaction never completed -- not scored as a
          // scoreboard failure here since the virtual sequence's own
          // timeout guard already flags that.
          `uvm_info("SB_IBI_EXTRA_DATA",
            $sformatf("[target %0d] Target was configured to send %0d extra IBI data byte(s) but none were sent (transaction likely did not complete)",
                      i, i3c_env_cfg_h.i3c_target_agent_cfg_h[i].ibi_num_extra_bytes), UVM_MEDIUM)
        end
      end
    end
    #1ns; // yield so this doesn't spin at zero simulation time
